...
public class DatabaseHelperReadHistory extends SQLiteOpenHelper{
    ...
    private static final String EPC = "EPCID";
...
//            contentValues.put(EPC, tag_id+ " "+  jsonObject.get("Timestamp").toString());


//            contentValues.put(EPC, tag_id);
            String item = null;
            ...
            contentValues.put(TIME, time+" "+item);
            contentValues.put(EPC, tag_id);
...
//            String strNew = str.replace("a", "");
            DatabaseReference itemsRef = bag_history_ref.child(time.replace(".","")+ " "+ item);
...
//            Log.i("Insert", result + "");
    ...
    public Cursor getItemID(String name){
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "SELECT " + EPC + " FROM " + NAME +
                " WHERE " + ITEM + " = '" + name + "'";
        Cursor data = db.rawQuery(query, null);
        return data;
    }
